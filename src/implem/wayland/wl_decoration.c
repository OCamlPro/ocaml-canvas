    ...
    for (int r = 0; r < face->glyph->bitmap.rows; ++r) {
      ...
      for (int c = 0; c < face->glyph->bitmap.width; ++c) {
        ...
        if (alpha == 255) {
          ...
          target[4 * d_idx] =
            (alpha * 255 + (255 - alpha) * (target[4 * d_idx])) / 255;
          target[4 * d_idx + 1] =
            (alpha * 255 + (255 - alpha) * (target[4 * d_idx])) / 255;
          target[4 * d_idx + 2] =
            (alpha * 255 + (255 - alpha) * (target[4 * d_idx])) / 255;
  ...
  const char *title)
{
  ...
  struct wl_buffer *wl_buffer =
    wl_create_buffer(width, _decor_height, &pool_data);
  ...
  const char *title)
{
  ...
  wl_decoration_t *decor =
    (wl_decoration_t *)calloc(1, sizeof(wl_decoration_t));
  if (decor == NULL) {
    return NULL;
  }
  ...
  _wl_decoration_button(decor, &decor->wl_closebutton_surface,
                        &decor->wl_closebutton_subsurface,
                        &decor->wl_closebutton_buffer, 30, 255, 0, 0);
  wl_subsurface_set_position(decor->wl_closebutton_subsurface, width - 40, 5);
  _wl_decoration_button(decor, &decor->wl_maxbutton_surface,
                        &decor->wl_maxbutton_subsurface,
                        &decor->wl_maxbutton_buffer, 30, 0, 255, 0);
  wl_subsurface_set_position(decor->wl_maxbutton_subsurface, width - 75, 5);
  _wl_decoration_button(decor, &decor->wl_minbutton_surface,
                        &decor->wl_minbutton_subsurface,
                        &decor->wl_minbutton_buffer, 30, 0, 0, 255);
