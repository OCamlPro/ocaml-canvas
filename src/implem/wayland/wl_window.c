  ...
  struct wl_array *states)
{
  ...
  struct wl_window_t *wl_window = (struct wl_window_t *)data;
  ...
  wl_array_for_each(p, states) {
    if (*p == XDG_TOPLEVEL_STATE_RESIZING) {
      ...
      if (wl_back->activated_window != NULL &&
  ...
  if (resizing && movedEnough) {
    ...
    // This check solves some issues when using the Ubuntu
    // compositor while having decorations activated
    if (wl_back->current_resize_edge & XDG_TOPLEVEL_RESIZE_EDGE_RIGHT ||
        wl_back->current_resize_edge & XDG_TOPLEVEL_RESIZE_EDGE_LEFT ||
        HAS_SERVER_DECORATION) {
      wl_window->base.width = clip_i32_to_i16(width);
    }
    if (wl_back->current_resize_edge & XDG_TOPLEVEL_RESIZE_EDGE_TOP ||
        wl_back->current_resize_edge & XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM ||
        HAS_SERVER_DECORATION) {
  ...
  int32_t height)
{
  ...
  if (window->base.decorated && !HAS_SERVER_DECORATION) {
    window->decoration =
      wl_decoration_create(window->wl_surface, width, title);
    wl_surface_set_user_data(window->decoration->wl_surface, window);
    wl_surface_set_user_data(window->decoration->wl_closebutton_surface,
                             window);
    wl_surface_set_user_data(window->decoration->wl_minbutton_surface,
                             window);
    wl_surface_set_user_data(window->decoration->wl_maxbutton_surface,
                             window);
  }
  ...
  wl_window_t *window)
{
  ...
  return wl_target_create(window->wl_surface);
}
  ...
  wl_window_t *window)
{
  ...
  if (!window->xdg_surface) {
    ...
    xdg_surface_add_listener(window->xdg_surface,
                             &_wl_xdg_surface_listener, &window);

    window->xdg_toplevel = xdg_surface_get_toplevel(window->xdg_surface);
    xdg_toplevel_add_listener(window->xdg_toplevel,
                              &_wl_xdg_toplevel_listener, window);
    ...
    if (window->base.decorated && HAS_SERVER_DECORATION) {
      window->server_decor =
        zxdg_decoration_manager_v1_get_toplevel_decoration(
          wl_back->zxdg_decoration_manager_v1,
          window->xdg_toplevel);
