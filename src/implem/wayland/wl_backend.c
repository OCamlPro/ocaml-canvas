  ...
  void)
{
  ...
  if (wl_back->focus_window->base.decorated == true &&
      wl_back->inside_decor_location == DECOR_REGION_BAR &&
      wl_back->mouse_posy < margin) {
    resize_edge |= XDG_TOPLEVEL_RESIZE_EDGE_TOP;
  } else if (wl_back->focus_window->base.decorated == false &&
             wl_back->mouse_posy < margin) {
    resize_edge |= XDG_TOPLEVEL_RESIZE_EDGE_TOP;
  } else if (wl_back->inside_decor_location == false &&
             wl_back->mouse_posy >
               wl_back->focus_window->base.height * 256 - margin) {
    resize_edge |= XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM;
  }

  if (wl_back->mouse_posx < margin) {
    resize_edge |= XDG_TOPLEVEL_RESIZE_EDGE_LEFT;
  } else if (wl_back->mouse_posx >
               wl_back->focus_window->base.width * 256 - margin) {
    resize_edge |= XDG_TOPLEVEL_RESIZE_EDGE_RIGHT;
  }

  wl_back->current_resize_edge = resize_edge;
}
...
{
  wl_back->cursor =
    wl_cursor_theme_get_cursor(wl_back->cursor_theme, cursor_name);
  ...
  wl_fixed_t y)
{
  ...
  if (surface != NULL) {
    ...
    if (wl_back->focus_window->base.decorated && !HAS_SERVER_DECORATION) {
      if (surface ==
          wl_back->focus_window->decoration->wl_surface) {
        wl_back->inside_decor_location = DECOR_REGION_BAR;
      } else if (surface ==
                 wl_back->focus_window->decoration->wl_closebutton_surface) {
        wl_back->inside_decor_location = DECOR_REGION_CLOSE_BUTTON;
      } else if (surface ==
                 wl_back->focus_window->decoration->wl_maxbutton_surface) {
        wl_back->inside_decor_location = DECOR_REGION_MAX_BUTTON;
      } else if (surface ==
                 wl_back->focus_window->decoration->wl_minbutton_surface) {
  ...
  uint32_t state)
{
  ...
  // When the focus screen is hidden with a key down event for example,
  // the key up event will still trigger. This check mitigates that.
  if (wl_back->focus_window &&
      wl_back->focus_window->base.visible == true &&
      wl_back->inside_decor_location == false) {
    wl_backend_t *wl_back = (wl_backend_t *)data;
    event_t evt;
    ...
    evt.desc.button.state =
      (state == WL_POINTER_BUTTON_STATE_PRESSED) ? BUTTON_DOWN : BUTTON_UP;
...
//    uint32_t resizeEdge = XDG_TOPLEVEL_RESIZE_EDGE_NONE;
    ...
    if (wl_back->current_resize_edge != XDG_TOPLEVEL_RESIZE_EDGE_NONE) {
      xdg_toplevel_resize(wl_back->focus_window->xdg_toplevel,
                          wl_back->seat, serial, wl_back->current_resize_edge);
    } else if (wl_back->inside_decor_location == DECOR_REGION_BAR) {
      xdg_toplevel_move(wl_back->focus_window->xdg_toplevel,
                        wl_back->seat, serial);
  ...
  enum wl_keyboard_key_state state)
{
  ...
  if (wl_back->focus_window && wl_back->focus_window->base.visible) {
    wl_backend_t *wl_back = (wl_backend_t *)data;
    uint32_t pressedKeyCharacter =
      xkb_state_key_get_utf32(wl_back->xkb_state, key + 8);
    event_t evt;
    evt.target = wl_back->focus_window;
    evt.time = wl_get_time();
    evt.type = EVENT_KEY;
    ...
    evt.desc.key.state =
      (state == WL_KEYBOARD_KEY_STATE_RELEASED) ? KEY_UP : KEY_DOWN;

    // Modifiers
    key_modifier_t mods = MOD_NONE;
    if (xkb_state_mod_name_is_active(wl_back->xkb_state, XKB_MOD_NAME_SHIFT,
                                     XKB_STATE_MODS_EFFECTIVE)) {
      mods |= MOD_SHIFT;
    }
    if (xkb_state_mod_name_is_active(wl_back->xkb_state, XKB_MOD_NAME_NUM,
                                     XKB_STATE_MODS_EFFECTIVE)) {
      mods |= MOD_NUMLOCK;
    }
    if (xkb_state_mod_name_is_active(wl_back->xkb_state, XKB_MOD_NAME_CAPS,
                                     XKB_STATE_MODS_EFFECTIVE)) {
      mods |= MOD_CAPSLOCK;
    }
    if (xkb_state_mod_name_is_active(wl_back->xkb_state, XKB_MOD_NAME_ALT,
                                     XKB_STATE_MODS_EFFECTIVE)) {
      mods |= MOD_ALT;
    }
    if (xkb_state_mod_name_is_active(wl_back->xkb_state, XKB_MOD_NAME_CTRL,
                                     XKB_STATE_MODS_EFFECTIVE)) {
      mods |= MOD_CTRL;
    }
    if (xkb_state_mod_name_is_active(wl_back->xkb_state, XKB_MOD_NAME_LOGO,
                                     XKB_STATE_MODS_EFFECTIVE)) {
      mods |= MOD_META;
    }
  ...
  uint32_t size)
{
  ...
  wl_back->xkb_keymap =
    xkb_keymap_new_from_string(wl_back->xkb_context,
                               map_shm, XKB_KEYMAP_FORMAT_TEXT_V1,
  ...
  void)
{
  ...
  /* Force sync so registry listener are run
     (this calls dispatch until sync processed) */
  wl_display_roundtrip(wl_back->display); /* Block until processed by server */
  ...
  /* Retrieve the keyboard, initiate xkb_context and add listener */
  wl_back->keyboard = wl_seat_get_keyboard(wl_back->seat);
  if (wl_back->keyboard) {
    wl_back->xkb_context = xkb_context_new(XKB_CONTEXT_NO_FLAGS);
    wl_keyboard_add_listener(wl_back->keyboard,
                             &_wl_keyboard_listener, wl_back);
  }
  ...
  uint32_t time)
{
  ...
  if (wl_callback) {
    ...
    wl_callback_add_listener(wl_window->wl_callback,
                             &wl_callback_listener,wl_window);
  }
  event_t evt;
  if (wl_window->pending_resize) {
    // resize decorations
    if (wl_window->base.decorated && !HAS_SERVER_DECORATION) {
      wl_decoration_resize(wl_window->decoration,
                           wl_window->base.width,
                           wl_window->title);
    }
    // resize surface
